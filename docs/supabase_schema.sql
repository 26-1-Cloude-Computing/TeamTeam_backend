...
-- 14. meeting 테이블
--     일정 페이지의 "회의 일정"을 영속 저장. 이 테이블이 없으면
--     GET/POST /api/teams/{id}/meetings 가 동작하지 않는다.
--     (2026-06-08 생성됨, RLS off — 다른 테이블과 동일 정책)
...
-- ============================================================
-- references (public) — 자료실 직접 파일 업로드 저장소.
--   POST /api/teams/{id}/references/upload 가 이 버킷에 파일을 올리고
--   공개 URL을 reference_room.file_url 에 저장한다.
--   대시보드 Storage 또는 Storage API로 생성 (2026-06-08 생성됨).
...
CREATE INDEX idx_meeting_team          ON meeting(team_id, date);