...
> 작성 기준: 2026-06-11
> **최신 변경(2026-06-10~11)**: 로그인 유지(크로스도메인 refresh), Gemini `gemini-2.5-flash` 실연동, 공지 고정·상호평가 DB·업무 담당자 재배정 API, AI 일정 2단계 워크플로우+인원별 할당, 다가오는 일정, 체감속도(SWR). 상세는 `../../CLAUDE_CONTEXT.md` 및 `recent_changes.md`.
> ⚠️ **AWS Academy 랩 중지 시 EC2도 stop됨** → 랩 재시작 후에도 `i-05ccd62f09320b9cb`가 `stopped`면 ALB 503. 데모 전 인스턴스 `running` 확인(없으면 EC2 start).
